...
- Public repo: no personal names, local paths, employer or project names in any file. Run the owner's hygiene grep (the personal-name, project-name and home-path pattern kept outside this repo) over the whole diff before every commit; it must print nothing.
...
def test_match_project_by_slug_and_alias(repo):
    ...
    assert mind.match_project(cfg, "acme-backend") == "acme-api"
...
def test_build_projects_index_and_reindex(repo):
    ...
    assert (cfg.home / "projects" / "index.md").read_text() == (
        ...
        "- acme-api | Acme-Api | python | Quotes from chat messages.\n"
    )
...
Run: the owner's hygiene grep over `git diff main --name-only` → must print nothing.
