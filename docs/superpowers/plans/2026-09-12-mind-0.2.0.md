...
- [ ] **Step 3b: Update the existing exact-string test** `test_validate_meta_reports_every_problem`: the enum error now reads `type must be one of: principle, preference, decision, procedure, gotcha, reference, precedence`. Any other test pinning the `TYPES` list gets the same suffix.
...
def cmd_propose(cfg: Config, drafts: list[Path], topic: str, scope: str, project: str | None,
    ...
    if not _has_upstream(cfg) and not remote_has:
        start = "HEAD"
    add_args = ["worktree", "add", "-q", "--", str(wt), start] if remote_has else ["worktree", "add", "-q", "-b", branch, "--", str(wt), start]
...
Test for the unpushed case: create a local `propose/2026-09-12-stuck` branch in `cfg.home` with one commit and no push; `cmd_proposals` returns `[("propose/2026-09-12-stuck", "(unpushed)")]` and the inject line reads `1 open proposal: propose/2026-09-12-stuck (unpushed)`.

`import json` at the top of `mind.py`. The `_current_branch`/`_checkout_main` helpers are not needed: the shared checkout is never switched. A failed push leaves the branch in the local repo (the worktree commit is on `refs/heads/<branch>` even after the worktree is removed). Parser: `propose` takes `drafts` (nargs `+`), `--topic` (required), `--scope` (choices, default `global`), `--project`, `--body`; `proposals` takes nothing and prints `<branch> <url>` per line or `mind: no open proposals`. `main` wires both after `ensure_checkout`.

- [ ] **Step 3b: Concurrency test.** While a `cmd_propose` runs (monkeypatch `_gh` to a function that, on `pr create`, calls `mind.cmd_inject(cfg, "compact", tmp_path)` and stores the output), the inject output must not contain the proposed note's title. Assert that literally.
- [ ] **Step 4: Run, expect pass** — [ ] **Step 5: Commit** `feat(mind): propose mode opens a PR per batch in a temporary worktree; proposals lists open branches`
...
- [ ] **Step 3b: Update the existing exact-string inject tests** (`test_cmd_inject_prints_sections_in_order` and any other that pins the full payload): the mode line `\nMode: apply notes silently and cite. Conflicts: use precedence notes.\n` follows the protocol; the protocol string gained its final sentence in this task. `test_cmd_inject_compact_makes_no_network_call` must also stub `cmd_proposals` (compact reads the cache, but the cache file may be absent: in that case nothing is printed and no network call is made).
...
- [ ] **Step 2: Run** — [ ] **Step 3: Implement** `_remote_reachable(cfg) -> tuple[bool, int|str]` runs `git ls-remote --heads origin` with a 10 s timeout and returns `(True, elapsed_ms)` or `(False, redacted last stderr line)`. `cmd_doctor` builds the nine lines from spec section 9: `identity:` prints `git config user.email` or `none, will use mind@localhost`; `gh:` parses the first token after `gh version` and appends ` authenticated` when `auth status` returns 0, ` present, not authenticated` otherwise, or `missing` when `_gh` returns None; `pending:` counts lines and `read_pending(cfg, None, 10**6)`; `notes:` counts accepted, draft, malformed via `_malformed_count`, and `len(list_projects(cfg))`. Every dynamic string passes `_redact(…, cfg)`. `doctor` never clones or mutates: `main` special-cases it like `reindex`, and a new read-only `_checkout_state(cfg) -> str` returns `ok` when `.git` exists and HEAD verifies, `missing` when `cfg.home/.git` is absent, `broken (no HEAD)` when `.git` exists but HEAD does not verify, or `broken (not a git dir)` when `rev-parse --git-dir` fails. Pinned test: a `cfg` whose home does not exist yields `checkout: missing` and `upstream: none`, `remote:` still probed, `notes: 0 accepted, 0 drafts, 0 malformed, 0 projects`; a home with an empty `.git` directory yields `checkout: broken (not a git dir)`.
...
## Impact-check rulings (2026-09-12)

Applied above: propose runs in a temporary worktree (the shared checkout never leaves `main`; concurrency test added); the two exact-string tests broken by the new type and the mode line are updated in Tasks 1 and 6; the ask skill appends rather than replaces. Report: `.dev/BEHAVIORAL_IMPACT_feat-mind-0.2.0.md`.

## Scenario-plan rulings (2026-09-12)

Applied above: string settings values parse as booleans and junk keeps the default; doctor uses a read-only `_checkout_state` with pinned strings; `proposals` lists unpushed local branches as `(unpushed)`. Critical rows in `.dev/test-plan-feat-mind-0.2.0.md` are implemented; Defensive rows are not (tier rule).
