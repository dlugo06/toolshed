...
`/dev-kit:ship` runs one of four reviewer tiers: **standard** (three Opus reviewers), **`--full`** (the same three, the security reviewer told what changed; auto-selected when the diff touches a dependency manifest, `.env.example`, migrations, or new egress), **`--fix`** (one Opus `pr-reviewer` in combined mode carrying the silent-failure and security checklists, for small single-subsystem bugs), or **`--light`** (docs/config only). `--reviewed` skips simplify and spec-check when a whole-branch review already ran. `/dev-kit:process-review` has `--autonomous` (rulings from stored preferences, decision table posted on the PR) and `--no-replies`. `/dev-kit:plan-tests` takes `--fix|--standard|--full` to bound the scenario count, and the bound is on tests written (plan-named plus Critical rows). `/dev-kit:check-impact` diffs a plan against the project's **behaviour register** (`docs/behaviour-register.md`, one row per user-observable rule; template in `templates/`) when one exists, reading only the specs the touched rows cite instead of the whole spec corpus; `--seed-register` creates it from the existing specs, and `ship` appends the rows each PR adds. Suites run once per change; agents read the diff once and write their report file before they finish. When the `mind` plugin is installed, rulings and findings cite a mind note ID (`[per <ID>]`) or are marked `[provisional]` instead of being decided from memory alone: `process-review --autonomous`'s decision table gains a `Basis` column, `ship`'s PR body Rulings section carries the suffix, and the four review/planning agents cite a settling note instead of filing a preference-shaped finding (correctness findings are never withheld on this basis). See §Mind in `plugins/dev-kit/README.md`.

### [`orchestrator`](plugins/orchestrator/) — 1.5.0

A cross-project work router. Reads every project's PRD items, assigns each one a pipeline stage and a tier (`fix` / `standard` / `full`, with subagent caps of 8 / 14 / 20 and a token guideline per tier), runs the single next transition by invoking only the skill or agent its stage table names, records the result on the item, and stops at the two human gates (merge, and `passes`). With the `gh` CLI installed, an item's stage is derived from evidence (the PR's reviews and the `.dev` review reports), never trusted as asserted; the SessionStart brief also says when the local default branch is behind origin. The reference (`plugins/orchestrator/reference/orchestrator.md`) holds the stage machine, the model table, the input contract for every dispatch, reference dispatch costs, and the guardrails (named-path staging, `--body-file`, hook-aware lookups, inline micro-fixes for one-file test-or-string changes, no attribution trailers unless the owner asks). When the `mind` plugin is installed, every ruling the orchestrator makes on the owner's behalf cites a settling mind note or is applied as `provisional` and proposed back to the data repo as one pull request at the end of the transition (§Consult the mind); without it, every consult is skipped and the report says `Mind: not installed`. `/orchestrator:release` is the merge gate's coordinator: it audits the PRs a run left open against the tier table from their artifacts, verifies the suites fresh per PR and per integration branch, batches by deploy surface, writes the owner's manual checks and opens the rollup PRs (merge commits, never squash), and audits which of the mind's proposed notes the owner kept before merging so the proposal rule keeps up (`plugins/orchestrator/reference/release.md`).

### [`mind`](plugins/mind/) — 0.2.1

Owner preference and decision store. A SessionStart hook pulls a private notes repo (`MIND_REPO`) and injects the global index, the current project's index, and the list of known projects; indexes are generated locally and gitignored in the data repo, never committed. `/mind:remember` writes a typed note the owner stated directly and pushes it; `/mind:ask` searches notes and cites IDs, with a `[precedence]` note surfaced first when two hits conflict. Anything an agent only inferred — a passive-capture digest, a seeding batch — goes out as a pull request instead: a `UserPromptSubmit` hook records prompts to a local pending file, `/mind:digest` classifies them and opens a proposal branch with `mind.py propose`, and `/mind:review` and `/mind:lint` keep the mind current (stale-note reinforce/weaken/revise/retire, and a report of malformed, duplicate, dangling, or budget-overflowing notes). `mind.py doctor` diagnoses a second machine or a cloud session. Inert without `MIND_REPO`.
...
plugins/<plugin-name>/            # one folder per plugin
  ...
  skills/                         # orchestrator: status, triage, next, advance, release
  reference/, scripts/, tests/    # orchestrator: the reference documents, status.py, release.py, their tests
