---
name: review-tests
description: Audit test quality for the current branch before shipping.
---
...
1. Launch the `test-quality-reviewer` agent (subagent_type: "dev-kit:test-quality-reviewer") in the foreground
...
4. Present the verdict to the user:
   - **PASS** → "Test quality review passed. Ready to `/dev-kit:ship`."
   - **NEEDS WORK** → Show the blocker list and ask: "Fix these issues before shipping, or override with `/dev-kit:ship` to skip."
