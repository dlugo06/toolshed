...
5. Present the scenario plan and the red flags to the user for review and approval. In an orchestrator run, the orchestrator rules on the red flags from the plan and the code and records the rulings; it does not dismiss any. Scenarios for inputs the call sites already guard against (a `None` URL where every caller checks `if url`) are ruled "not implemented, YAGNI" with the guard cited, not added; a ruling that changes a predicate names one fixture it must accept and one it must reject.
