...
- At the end of the transition, each provisional ruling is tested before it becomes a note [per PRIN-ID-001]: could the owner have said it before this item existed, without knowing the codebase, the library or the run? A stance on how software should be built passes. A fact that only makes sense with the item, the library or the environment in front of you fails, and so does any correction the orchestrator made to its own work along the way; those stay on the progress line and go wherever the project keeps that kind of fact (behaviour register, code comment, docs, PR body). Only a ruling that passes is drafted: one markdown file per note in the session scratch directory, frontmatter `title`, `type`, `stage`, `strength`, `source: orchestrator ruling <project>/<item> <date>` (and `scope: project:<slug>` for a project fact — omit `id`, which `propose` assigns), a one- or two-sentence body plus `**Why:**` and `**How to apply:**`; scope by "true in another repository". One draft per ruling, batched into a single pull request on the data repo: `python3 "$MIND" propose <drafts...> --topic <project>-<item>-<stage> [--scope project --project <slug>]`. The PR URL goes on the progress line and in the report.
