...
- At the end of the transition, each provisional ruling is tested before it is drafted [per PRIN-ID-001]: could the owner have stated it before this item existed, without the codebase, the library or the run in front of them? Only a stance on how software should be built passes. Five classes fail and stay on the progress line, in the register, the docs or the PR body: an **item ruling** (one item's scope, YAGNI or trade-off decision generalised: the Why narrates the item), **run mechanics** (worktrees, missing keys, hook denials, a fixture the run could not capture), the agent's **own correction** (a bug it introduced and fixed in the same run, written as a principle), **code placement** (where a function lives or how a module is shaped in this codebase), and a **library trap** (a third-party behaviour learned during the run). The owner dropped every note of those classes from proposal PRs #4 through #14 on the data repo and kept the rest; the `release` skill re-checks the classes after every merge. A ruling that passes becomes a note: draft it as one markdown file per note in the session scratch directory, frontmatter `title`, `type`, `stage`, `strength`, `source: orchestrator ruling <project>/<item> <date>` (and `scope: project:<slug>` for a project fact — omit `id`, which `propose` assigns), a one- or two-sentence body plus `**Why:**` and `**How to apply:**`; scope by "true in another repository". One draft per ruling, batched into a single pull request on the data repo: `python3 "$MIND" propose <drafts...> --topic <project>-<item>-<stage> [--scope project --project <slug>]`. The PR URL goes on the progress line and in the report.
