...
- Jest's "worker process has failed to exit gracefully" is pre-existing on this project's bridge suite; say so instead of counting it. `npm test` run from the main checkout also collects every harness worktree's copy of the suite under `.claude/worktrees/*/tests` (ten worktrees turned 348 tests into 3194); run jest in a scratch worktree, or read the count from the last batch that changed a bridge file.
