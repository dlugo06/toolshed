...
Only note files and `project.md` are authored. Every `index.md` is regenerated by the script after each write; concurrent sessions on different machines never hand-edit the same hot file. Git history is the change log; there is no `log.md`.
...
It prints, in order: the protocol line, the global index, the current project's index (or "no notes for `<candidate>` yet"), the projects index, and the draft count when non-zero. The three indexes together are capped at 4,000 characters (`INDEX_BUDGET`): the global index is truncated first, then the project index, and the projects index is never truncated.
...
| `ask <terms...> [--all] [--project slug] [--drafts]` | case-insensitive term match over title and body of accepted notes in global plus the current project (`--all`: every project); rank by terms hit; print up to 10 rows |
...
- Authentication: SSH on the owner's machines. In cloud sessions `MIND_REPO` is an https URL and `MIND_TOKEN` is set; the token is passed to git through a credential helper on the command line, never written to disk.
...
- A rebase conflict during `sync` (two machines touched the same file) aborts the rebase cleanly and reports the offline line; `add`'s own ID-collision retry backs out, re-pulls, and takes the next free ID.
